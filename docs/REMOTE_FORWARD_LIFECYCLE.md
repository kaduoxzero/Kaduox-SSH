...
The CLI retains all managed remote-forward handles until the requested command/tunnel finishes. It then closes remote forwards before disconnecting the SSH session. Local and dynamic listener handles are also released during the same shutdown sequence.
...
CLI 会在命令或 tunnel 生命周期内持有所有 managed remote-forward handle，并在 SSH session disconnect 之前逐个关闭远程转发。本地和动态 listener handle 也在同一 shutdown 阶段释放。
