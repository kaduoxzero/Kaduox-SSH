...
describe('HostSidebar', () => {
  ...
  it('separates dedicated jumps from targets and supports empty, renamed and collapsed folders', async () => {
    ...
    render(<HostSidebar hosts={[hosts[0], { ...hosts[1], role: 'jump' }]} sessions={[]} selectedAlias={null} search="" folders={[{ name: '空文件夹', role: 'target' }]} onSaveFolder={save} onSelect={vi.fn()} onConnect={vi.fn()} onEdit={vi.fn()} onAdd={vi.fn()} />)
    ...
    await waitFor(() => expect(save).toHaveBeenCalledWith('开发（内网）', null, 'target'))
    ...
    fireEvent.click(screen.getByRole('button', { name: '保存文件夹' }))
    await waitFor(() => expect(save).toHaveBeenCalledWith('新名称', '空文件夹', 'target'))
  })
})
