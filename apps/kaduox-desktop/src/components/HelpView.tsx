...
export function HelpView() {
  ...
  return <main className="help-view">
    ...
    <section><h2>信息、文件、记录和 Kaduox AI</h2><p>信息页打开后自动采集当前展示机器的 CPU、GPU、内存、磁盘和网络，每 10 秒刷新。SFTP 与终端共享连接。运行记录每页 50 条，记录显式执行的命令；不会记录终端按键或密码提示。</p><p>Kaduox AI 调用你配置的兼容厂商接口，可获取模型列表或手填模型。API 密钥保存在系统凭据库；发送主机上下文需要勾选授权。AI 发起的命令是否自动执行取决于权限模式：严格批准模式下所有命令都需你批准；请求批准（默认）模式下只读命令自动执行、修改/删除需批准；全部权限模式下只读与修改自动执行。删除类命令在任何模式下都需你批准，危险命令始终被拦截。不要向 AI 粘贴密码、私钥或令牌。</p></section>
