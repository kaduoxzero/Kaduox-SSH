...
export function HistoryView({ sessions, selectedAlias }: { sessions: Session[]; selectedAlias: string | null }) {
  ...
  return <main className="content-view history-view">
    <div className="view-heading"><div><span className="eyebrow">PERSISTENT RUN HISTORY</span><h1>运行记录</h1><p>显式执行的命令永久保存在本机，每页 50 条，支持按天查询、复制和编辑后重跑。记录不可清空，只能创建归档快照备份。终端里手敲的命令收录在会话栏的“命令历史”中（密码等敏感输入不记录）。</p></div><button className="secondary-button" onClick={() => void archive()} disabled={!data.total || running}>创建归档快照</button></div>
