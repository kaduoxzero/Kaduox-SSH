...
v0.31 adds OpenSSH `%l` and `%L` Include expansion from the platform-native local hostname. `%l` uses the full hostname returned by `gethostname`; `%L` uses the same per-argument hostname snapshot truncated at the first dot. Unix calls `gethostname` directly, while Windows performs a balanced Winsock 2.2 startup/cleanup around Winsock `gethostname` rather than approximating the value with an environment variable. Hostname lookup/non-UTF-8 failures remain fail-closed, and `${ENV}` substitution bytes are still never rescanned as percent tokens.
